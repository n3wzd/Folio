...
#### 문제
안드로이드 11(API 레벨 30)부터 적용된 Scoped Storage로 인해 이전의 파일 접근 방식이 차단됩니다.
...
#### 문제
앱이 안드로이드 백그라운드에서 실행될 때 약 15분 후에 강제로 종료되는 현상이 발생합니다.
...
#### 해결
`sqflite`는 동시 읽기/쓰기 작업을 지원하지 않으므로 첫 파일 생성 및 초기화 시에는 다른 작업을 제한해야 합니다.

데이터베이스 상태를 체크하는 프로퍼티를 추가하여 초기화 중에는 CRUD 연산을 제한합니다.
